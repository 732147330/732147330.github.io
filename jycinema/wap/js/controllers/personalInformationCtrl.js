 ...
 */
'use strict';
app.controller('personalInformationCtrl', function($state,httpService,$ionicHistory,$timeout,$ionicPopup,$ionicPlatform,$ionicBackdrop,$ionicActionSheet,JYApi,$scope,$interval,$rootScope,$ionicLoading,$ionicGesture) {
    ...
    var now = new Date();
    var max = new Date(now.getFullYear() + 100, now.getMonth(), now.getDate());
    $scope.settings = {
        theme: 'ios',
        lang: 'zh',
        display: 'bottom',
    ...
    $scope.getLocalTime=function(obj) {
        var d =  new Date(obj);
    ...
    function getDate (strTime) {
        ...
        return date.getFullYear()+"/"+$scope.formatDate(date.getMonth()+1)+"/"+$scope.formatDate(date.getDate());
    }
