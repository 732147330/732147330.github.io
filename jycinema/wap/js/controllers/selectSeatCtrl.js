 ...
 */
'use strict';
app.controller('selectSeatCtrl', function ($scope, $rootScope, $ionicViewSwitcher, httpService, $http, JYApi, myhrefService, $interval, $stateParams, $state, $ionicPopup, $ionicScrollDelegate, $ionicSlideBoxDelegate, $timeout, $filter, $ionicModal, $ionicBackdrop) {
    ...
    $scope.showNext = function () {
        if ($scope.selSeatData.length > 0) {
            ...
            $scope.HourMin = $scope.formatTimeHM($scope.currentScheduleData[$scope.curIndex].showtime);
            $scope.HourMinUp = $scope.formatTimeHM($scope.currentScheduleData[$scope.curIndex - 1].showtime);
            console.log($scope.currentScheduleData.length)
    ...
    $scope.weixinLogin = function () {
        location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx5d69bdaaa9765cda&redirect_uri='
            + encodeURIComponent(locals.returnUrl() + '/#/selectSeat?movieId=' + $scope.movieId + '&theatreId=' + $scope.theatreId
                + '&scheduleTime=' + $scope.scheduleTime + '&curIndex=' + $scope.curIndex + '&filmIdAlt=' + $scope.filmIdAlt)
