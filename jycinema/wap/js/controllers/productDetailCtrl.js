 ...
 */
'use strict';
app.controller('productDetailCtrl', function ($scope, $ionicViewSwitcher, $rootScope, $ionicModal, $stateParams, $http, $interval, $ionicBackdrop, $timeout, $ionicPopup, $state, $location, httpService, JYApi, $ionicScrollDelegate) {
  $scope.address = {}
  ...
  $scope.ruleHeaderIdList = $stateParams.ruleHeaderIdList || [];
  if ($scope.skuId == 6003842) {
    $state.go("productDetail", {skuId: 6324564, urlParams: JSON.stringify($stateParams)})
  }
  ...
  $scope.getDatabyskuParams = function () {
    ...
    $scope.selectArr = [];
    console.log($scope.attributesData)
  ...
  $scope.weixinLogin = function () {
    location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx5d69bdaaa9765cda&redirect_uri='
      + encodeURIComponent(locals.returnUrl() + '/#/productDetail?skuId=' + $scope.skuId+'&ruleHeaderIdList=' + $scope.ruleHeaderIdList)
  ...
  $scope.goLogin = function () {
    $scope.needBind = false;
    $state.go('login', {viewName: 'productDetail', urlParams: JSON.stringify({skuId: $scope.skuId})});
    $ionicViewSwitcher.nextDirection("none")
  };
