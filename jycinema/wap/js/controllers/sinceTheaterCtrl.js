 ...
 */
'use strict';
app.controller('sinceTheaterCtrl', function ($scope, $rootScope, $stateParams, $ionicViewSwitcher, $ionicPopup, $ionicLoading, $http, $timeout, $state, httpService, $interval, JYApi, $location, $ionicPlatform, $ionicModal, $ionicBackdrop) {
        ...
        if ($stateParams.status == "edit") {
            ...
            httpService.getData(JYApi.findItemSkuCopy, 'post', {
                params: JSON.stringify({
                    pageIndex: 1,
                    pageRows: 10,
                    itemId: $scope.cinemaBySkuData[index].itemId ? $scope.cinemaBySkuData[index].itemId : '',
                    cinemaOuterId: $scope.cinemaBySkuData[index].cinemaOuterId ? $scope.cinemaBySkuData[index].cinemaOuterId : ''
                })
            }, function (res) {
                if (res.status == "S") {
    ...
    $scope.weixinLogin = function () {
        location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx5d69bdaaa9765cda&redirect_uri='
            + encodeURIComponent(locals.returnUrl() + '/#/sinceTheater?itemId=' + $scope.itemId + '&skuId=' + $scope.skuId)
