    ...
    $scope.buy = function () {
        ...
        if (!localStorage.userInfo) {
            ...
            if (!localStorage.hasAddress) {
                ...
                sessionStorage.selectArr = JSON.stringify($scope.selectArr);
                sessionStorage.productDetailData = JSON.stringify($scope.productDetailData);
                let ruleHeaderIdList = '';
                let skuId = '';
                let cardId = '';
                let cardRenewalFlag = 'N'
                if($stateParams.ruleHeaderIdList != undefined) {
                    ruleHeaderIdList = $stateParams.ruleHeaderIdList;
                }
                if($stateParams.skuId != undefined) {
                    skuId = $stateParams.skuId;
                }
                if($stateParams.cardId != undefined) {
                    cardId = $stateParams.cardId;
                }
                if($stateParams.cardRenewalFlag != undefined) {
                    cardRenewalFlag = $stateParams.cardRenewalFlag;
                }
                //去生成订单详情页
                $state.go('mallOrder', {status: 'product',ruleHeaderIdList: ruleHeaderIdList,skuId: skuId,cardId: cardId,cardRenewalFlag: cardRenewalFlag});
