'use strict';
app.controller('mealListCtrl', function ($scope, $rootScope,$timeout,$ionicViewSwitcher,httpService,JYApi,$interval,$stateParams,$ionicPopup,$state,$ionicScrollDelegate,$ionicBackdrop,$ionicModal) {
      ...
      angular.forEach(v.itemGoodsList,function(value,key) {
        if (value.salesQty) {
          $scope.params.push({
            ...
            screenName: $stateParams.screenName ? $stateParams.screenName : '',
            seatDeatilMessage: $stateParams.seatDeatilMessage ? $stateParams.seatDeatilMessage : '',
            scanCodeFlag: $stateParams.scanCodeFlag == 1 ? 'ORDER_FOR_GOODS' : '',
  ...
  $scope.findScheduleProductList=function () {
    httpService.getData(JYApi.findScheduleProductList, 'post', {
      params:JSON.stringify({
        ...
        "typeOut": "ITEM_GOODS",
        "skuItemType": "GROUND_GOODS",
        purchaseType:$stateParams.scanCodeFlag?'Y':'',
        cinemaOuterId:$stateParams.cinemaOutId,
        pageIndex:1,
        pageRows:100
      })
