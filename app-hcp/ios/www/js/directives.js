angular.module('jyApp.directives', [])
    ...
    .directive('tabsClick',function ($ionicScrollDelegate) {
        return {
            restrict: 'A',
            link: function (scope, element, attrs) {
                ...
                element.on('click','i',function(e){
                    ...
                    if(scope.selSeatData.length==6 && curData.flag!='Y'){
                        var myPopup = $ionicPopup.show({
                            ...
                            template: languageSetting.maxSelect+'!'
                        });
                        $timeout(function(){
                            "use strict";
                            myPopup.close();
    ...
    .directive('checkLogin',function ($state) {
        return {
            restrict: 'A',
            link: function (scope, element, attrs) {
                element.find('td').bind('click',function(){
                    ...
                    if($(this).attr('ref')){
                        if(isLogin){
                            if(goLink=='myVipHome' && !JSON.parse(localStorage.userInfo).mobileNumber){
